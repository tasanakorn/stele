> **Update (2026-04, v0.16.0 / PRD-020):** session KV, project KV, phase state, storage, event log, and session lifecycle have moved from stele-server HTTP to a **local SQLite DB** at `~/.local/share/steop/steop.db` (override via `STEOP_DB`). The migrated REST routes (`/api/v1/steop/{state,storage,status,session,log}.*`) and tables (`steop_sessions`, `steop_state`, `steop_storage_*`, `steop_log`, `steop_counters`) were removed from stele-server; `steop_mailbox` is the only surviving `steop_*` server table. Only `steop.mailbox.*` and `steop.notify` remain on stele-server HTTP. References below to `/api/v1/steop/state`, `/api/v1/steop/storage`, `/api/v1/steop/state/:id/incr`, the `steop_state` / `steop_storage` / `steop_counters` tables, and any "v1 API freeze" claim are **historical** — treat them as a description of the v1 shape that PRD-020 superseded. See [docs/steop/local-storage.md](./local-storage.md) for the current layout.
>
