...
| Statusline / HUD                       | `statusline.sh` + `cerbrix hud render` shows mode/phase/N/M/L/R    | `steop statusline` one-shot renderer + `/steop:statusline-setup` skill that patches `~/.claude/settings.json`   | B      | ✓ v1   | Cerbrix-style integration: rendered by Claude Code itself |
...
- **Statusline / HUD** — ✓ **v1**. `/api/v1/steop/status/:id` projects the statusline shape and never 404s. The integration mirrors cerbrix's two-layer split: line 1 comes from `~/.claude/statusline.sh` (whatever renderer the user already has — cerbrix, custom, or a minimal fallback written by steop on first install), line 2 comes from `steop statusline`. `/steop:statusline-setup` appends an idempotent `steop statusline` block between sentinel markers at the bottom of `~/.claude/statusline.sh` — re-running refreshes the block, and the uninstall path removes it cleanly without touching the user's line 1. The user's `~/.claude/settings.json` `statusLine` key is patched to point at `~/.claude/statusline.sh`. There is no plugin-level `statusLine` field in `plugin.json` — Claude Code only exposes `statusLine` via user settings, not plugin manifests.
...
| 12  | `steop statusline` one-shot renderer + settings.json setup skill              | Medium | Low    | ✓ v1    | Reads Claude Code stdin JSON; wired via `/steop:statusline-setup` (patches user's `~/.claude/settings.json`). |
