---
name: st-explore
description: Explore phase of the workflow chain. Uses Haiku (simple) or Sonnet (standard) to research the codebase and gather context for a task. Use when the user wants to investigate code before planning.
---
...
If the task spans multiple independent areas of the codebase, launch multiple Explore agents in parallel — one per area. For example, if exploring both the API layer and storage layers, launch two agents concurrently. Combine their findings into a single summary.

### Exploration Goals

The exploration agent(s) should:
